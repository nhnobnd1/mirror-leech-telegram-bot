...
aria2c --allow-overwrite=true --auto-file-renaming=true --bt-enable-lpd=true --bt-detach-seed-only=true \
       ...
       --max-connection-per-server=10 --max-concurrent-downloads=2 --max-file-not-found=0 --max-tries=20 \
       --min-split-size=10M --optimize-concurrent-downloads=true --peer-id-prefix=-qB4510- --reuse-uri=true \
       --peer-agent=qBittorrent/4.5.1 --quiet=true --rpc-max-request-size=1024M --seed-ratio=0 --split=10 \
